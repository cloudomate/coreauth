...
export default function OrgLogin() {
  ...
  useEffect(() => {
    const loadOrgData = async () => {
      try {
        ...
        const orgRes = await api.get(`/organizations/by-slug/${orgSlug}`);
        setOrganization(orgRes.data);

        // Fetch SSO connections for this organization
        const connRes = await api.get(`/oidc/providers?tenant_id=${orgRes.data.id}`);
  ...
  const handleSSOLogin = async (connection) => {
    try {
      ...
      const response = await api.get('/oidc/login', {
        params: {
