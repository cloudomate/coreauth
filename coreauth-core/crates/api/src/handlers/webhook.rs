...
pub async fn list_webhooks(
    ...
    let webhooks: Vec<WebhookRow> = sqlx::query_as(
        r#"
        SELECT id, tenant_id as organization_id, name, url, events, is_enabled,
...
pub async fn create_webhook(
    ...
    let webhook: WebhookRowWithSecret = sqlx::query_as(
        r#"
        INSERT INTO webhooks (tenant_id, name, url, secret, events, is_enabled, retry_policy, custom_headers)
        VALUES ($1, $2, $3, $4, $5, $6, $7, $8)
        RETURNING id, tenant_id as organization_id, name, url, secret, events, is_enabled,
                  retry_policy, custom_headers,
                  total_deliveries, successful_deliveries, failed_deliveries,
                  last_triggered_at, last_success_at, last_failure_at,
                  created_at, updated_at
...
pub async fn update_webhook(
    ...
    let webhook: WebhookRow = sqlx::query_as(
        ...
        UPDATE webhooks SET
            ...
            updated_at = NOW()
        WHERE id = $1 AND tenant_id = $2
        RETURNING id, tenant_id as organization_id, name, url, events, is_enabled,
                  retry_policy, custom_headers,
                  total_deliveries, successful_deliveries, failed_deliveries,
                  last_triggered_at, last_success_at, last_failure_at,
                  created_at, updated_at
...
pub async fn test_webhook(
    ...
    let webhook: WebhookRowWithSecret = sqlx::query_as(
        r#"
        SELECT id, tenant_id as organization_id, name, url, secret, events, is_enabled,
               retry_policy, custom_headers,
               total_deliveries, successful_deliveries, failed_deliveries,
               last_triggered_at, last_success_at, last_failure_at,
               created_at, updated_at
...
pub async fn get_delivery(
    ...
    let delivery: WebhookDelivery = sqlx::query_as(
        ...
        WHERE d.id = $1 AND d.webhook_id = $2 AND w.tenant_id = $3
        "#,
    )
    .bind(delivery_id)
    .bind(webhook_id)
