...
pub async fn verify_totp_with_token(
    ...
    let (org_slug, role) = if let Some(org_id) = org_id {
        let membership = sqlx::query_as::<_, (String, String)>(
            ...
            FROM tenants o
            JOIN tenant_members om ON om.tenant_id = o.id
...
#[derive(Debug, Serialize)]
pub struct SmsMfaEnrollResponse {
    pub method_id: Uuid,
    pub method_type: String,
    pub phone_number: String,
...
pub async fn enroll_sms(
    State(state): State<Arc<AppState>>,
    Extension(auth_user): Extension<AuthUser>,
    Json(request): Json<EnrollSmsRequest>,
) -> Result<Json<SmsMfaEnrollResponse>, (StatusCode, Json<ErrorResponse>)> {
    ...
    // Hash the code
    let code_hash = ciam_auth::password::PasswordHasher::hash(&code).map_err(|e| {
        (
            StatusCode::INTERNAL_SERVER_ERROR,
            Json(ErrorResponse::new("hash_error", &e.to_string())),
        )
    ...
    let otp_id: Uuid = sqlx::query_scalar(
        r#"
        INSERT INTO sms_otp_codes (user_id, phone_number, code_hash, expires_at)
        VALUES ($1, $2, $3, $4)
        RETURNING id
        "#,
    )
    ...
    if let Some(ref sms_service) = state.sms_service {
        sms_service.send_otp(phone, &code).await.map_err(|e| {
            tracing::error!("Failed to send SMS OTP: {}", e);
            (
                StatusCode::INTERNAL_SERVER_ERROR,
                Json(ErrorResponse::new("sms_send_error", "Failed to send SMS")),
            )
        })?;
    } else {
        // For development: log the code
        tracing::warn!("📱 [SMS OTP] Phone: {} Code: {}", phone, code);
    }

    // Create unverified MFA method
    let method_id = sqlx::query_scalar::<_, Uuid>(
        r#"
        INSERT INTO mfa_methods (user_id, method_type, phone_number, verified)
...
/// Verify SMS OTP and activate MFA method
/// POST /api/mfa/sms/:method_id/verify
pub async fn verify_sms(
    ...
    // Get the MFA method
    let phone_number: String = sqlx::query_scalar(
        r#"
        SELECT phone_number
        FROM mfa_methods
        WHERE id = $1 AND user_id = $2 AND method_type = 'sms' AND verified = false
        "#,
    )
    .bind(method_id)
    .bind(auth_user.user_id)
    ...
    // Get the latest OTP code for this user and phone
    let otp: Option<(Uuid, String, i32, i32, chrono::DateTime<chrono::Utc>)> = sqlx::query_as(
        r#"
    ...
    let (otp_id, code_hash, attempts, max_attempts, expires_at) = otp.ok_or_else(|| {
        (
            StatusCode::NOT_FOUND,
            Json(ErrorResponse::new("otp_not_found", "No OTP code found")),
        )
    })?;
    ...
    if expires_at < chrono::Utc::now() {
        return Err((
            StatusCode::BAD_REQUEST,
            Json(ErrorResponse::new("otp_expired", "OTP code has expired")),
        ));
    }
    ...
    // Verify the code
    let is_valid = ciam_auth::password::PasswordHasher::verify(&request.code, &code_hash)
        .map_err(|e| {
            (
                StatusCode::INTERNAL_SERVER_ERROR,
                Json(ErrorResponse::new("verification_error", &e.to_string())),
    ...
    if !is_valid {
        // Increment attempts
        sqlx::query(
            "UPDATE sms_otp_codes SET attempts = attempts + 1 WHERE id = $1",
        )
        .bind(otp_id)
        .execute(state.auth_service.db.pool())
        .await
    ...
    sqlx::query("UPDATE sms_otp_codes SET verified = true WHERE id = $1")
        .bind(otp_id)
        .execute(state.auth_service.db.pool())
        .await
        .ok();
    ...
    // Enable MFA for the user and update phone verification
    sqlx::query(
        r#"
        UPDATE users
        SET mfa_enabled = true, phone = $1, phone_verified = true, phone_verified_at = NOW()
        WHERE id = $2
        "#,
    )
    .bind(&phone_number)
...
/// Resend SMS OTP code
/// POST /api/mfa/sms/:method_id/resend
pub async fn resend_sms_otp(
    State(state): State<Arc<AppState>>,
    Extension(auth_user): Extension<AuthUser>,
    Path(method_id): Path<Uuid>,
) -> Result<Json<SmsMfaEnrollResponse>, (StatusCode, Json<ErrorResponse>)> {
    ...
    // Send SMS with OTP code
    if let Some(ref sms_service) = state.sms_service {
        sms_service.send_otp(&phone_number, &code).await.map_err(|e| {
            tracing::error!("Failed to send SMS OTP: {}", e);
            (
                StatusCode::INTERNAL_SERVER_ERROR,
                Json(ErrorResponse::new("sms_send_error", "Failed to send SMS")),
        ...
        // For development: log the code
        tracing::warn!("📱 [SMS OTP] Phone: {} Code: {}", phone_number, code);
    }
