...
impl Branding {
    ...
    pub fn has_terms(&self) -> bool {
        !self.terms_url.is_empty()
    }
    pub fn has_privacy(&self) -> bool {
        !self.privacy_url.is_empty()
    }
    pub fn has_footer_links(&self) -> bool {
        self.has_terms() || self.has_privacy()
    }
}
...
pub async fn login_submit(
    ...
    // Organization ID from explicit authorize request parameter (org-specific login)
    let explicit_organization_id = auth_request.organization_id;
    ...
    // Use explicit org from authorize request, fall back to user's default tenant
    let organization_id = explicit_organization_id.or(user.default_tenant_id);

    // If an explicit organization was requested, verify user membership
    if let Some(org_id) = explicit_organization_id {
        let is_member = sqlx::query_scalar::<_, bool>(
            "SELECT EXISTS(SELECT 1 FROM tenant_members WHERE user_id = $1 AND tenant_id = $2)",
        )
        .bind(user.id)
        .bind(org_id)
        .fetch_one(state.auth_service.db.pool())
    ...
    if !user.email_verified {
        // Look up the application's owning tenant to check security settings
        let app_tenant_id: Option<Uuid> = sqlx::query_scalar(
            "SELECT tenant_id FROM applications WHERE client_id = $1",
        )
        .bind(&auth_request.client_id)
        .fetch_optional(state.auth_service.db.pool())
        .await
        ...
        let require_verification = if let Some(tenant_id) = app_tenant_id {
        ...
        if require_verification {
            // Resend verification email, using the app's origin for the verification link
            if let Some(tenant_id) = app_tenant_id {
                let email = form.email.clone();
                let verification_service = state.verification_service.clone();
                let uid = user.id;
                let app_base_url = extract_origin(&auth_request.redirect_uri);
                tokio::spawn(async move {
                    if let Err(e) = verification_service
                        .send_verification_email(uid, tenant_id, &email, email.split('@').next().unwrap_or(&email), app_base_url.as_deref())
                        .await
                    {
...
pub async fn signup_submit(
    ...
    // Look up the application's owning tenant — new users belong to this tenant
    let app_tenant_id: Option<Uuid> = sqlx::query_scalar(
        "SELECT tenant_id FROM applications WHERE client_id = $1",
    )
    .bind(&auth_request.client_id)
    ...
    // Explicit org from authorize request, or the app's owning tenant
    let organization_id = auth_request.organization_id.or(app_tenant_id);

    // Check if email verification is required for this tenant
    let require_email_verification = if let Some(tenant_id) = app_tenant_id {
        let settings_json: Option<(serde_json::Value,)> = sqlx::query_as(
            "SELECT settings FROM tenants WHERE id = $1",
        )
        .bind(tenant_id)
    ...
    // Create user with default_tenant_id set to the app's owning tenant
    let user_id = Uuid::new_v4();
    if let Err(e) = sqlx::query(
        r#"
        INSERT INTO users (id, default_tenant_id, email, password_hash, metadata, is_active)
        VALUES ($1, $2, $3, $4, '{}', true)
        "#,
    )
    .bind(user_id)
    ...
    // Add user as member of the tenant
    if let Some(tenant_id) = app_tenant_id {
        if let Err(e) = sqlx::query(
            "INSERT INTO tenant_members (user_id, tenant_id, role) VALUES ($1, $2, 'member') ON CONFLICT DO NOTHING",
        )
        .bind(user_id)
        .bind(tenant_id)
        .execute(state.auth_service.db.pool())
        .await
        {
            error!("Failed to add user to tenant: {}", e);
        }
    }

    info!(user_id = %user_id, email = %form.email, tenant = ?app_tenant_id, "New user registered via Universal Login");

    // If email verification is required, send verification email and show check-email page
    if require_email_verification {
        if let Some(tenant_id) = app_tenant_id {
            ...
            tokio::spawn(async move {
                if let Err(e) = verification_service
                    .send_verification_email(user_id, tenant_id, &email, email.split('@').next().unwrap_or(&email), app_base_url.as_deref())
                    .await
                {
                    error!("Failed to send verification email: {}", e);
                }
    ...
    // Complete authorization (no email verification required)
...
pub async fn mfa_verify(
    ...
    let organization_id = user.as_ref().and_then(|u| u.default_tenant_id);
...
async fn get_org_branding(state: &Arc<AppState>, org_id: Uuid) -> (String, Branding) {
    ...
    .await
    {
        let branding = Branding {
            ...
            background_color: org.settings.branding.background_color.clone().unwrap_or_else(|| "#f3f4f6".to_string()),
            custom_css: org.settings.branding.custom_css.clone().unwrap_or_default(),
            terms_url: org.settings.branding.terms_url.clone().unwrap_or_default(),
            privacy_url: org.settings.branding.privacy_url.clone().unwrap_or_default(),
        };
        (org.name, branding)
    } else {
        ("CoreAuth".to_string(), Branding::default())
...
async fn handle_authenticated_user(
    ...
    // Create response with session cookie (303 See Other for POST→GET redirect)
...
/// e.g., "http://localhost:5050/oidc/callback" → Some("http://localhost:5050")
fn extract_origin(url: &str) -> Option<String> {
    // Find "://" to split scheme from rest
    let scheme_end = url.find("://")?;
    let after_scheme = &url[scheme_end + 3..];
    // The origin ends at the first "/" after the authority (host:port)
    let authority_end = after_scheme.find('/').unwrap_or(after_scheme.len());
    Some(format!("{}{}", &url[..scheme_end + 3], &after_scheme[..authority_end]))
}
...
// ============================================================================
// EMAIL VERIFICATION (click from email link)
// ============================================================================
...
/// GET /verify-email?token=xxx — handles the link clicked from the verification email.
pub async fn verify_email_page(
    State(state): State<Arc<AppState>>,
    Query(query): Query<VerifyEmailTokenQuery>,
) -> Response {
    let result = state.verification_service.verify_email(&query.token).await;

    let (success, error_message) = match result {
        Ok(_) => (true, String::new()),
        Err(e) => {
            let msg = match &e {
                ciam_auth::AuthError::InvalidToken(_) => "This verification link is invalid or has already been used.".to_string(),
                ciam_auth::AuthError::TokenExpired => "This verification link has expired. Please request a new one.".to_string(),
    ...
    let template = EmailVerifiedTemplate {
        success,
        error_message,
    };
