        ...
        match (f.attribute.as_str(), &f.operator) {
            ("userName", ScimFilterOp::Eq) | ("emails.value", ScimFilterOp::Eq) => {
                let users: Vec<UserRow> = sqlx::query_as(
                    ...
                    FROM users u
                    JOIN tenant_members om ON om.user_id = u.id
                    WHERE om.tenant_id = $1 AND u.email = $2
                    "#,
                )
                .bind(org_id)
                .bind(&f.value)
            ...
            ("externalId", ScimFilterOp::Eq) => {
                let users: Vec<UserRow> = sqlx::query_as(
                    ...
                    FROM users u
                    JOIN tenant_members om ON om.user_id = u.id
                    WHERE om.tenant_id = $1 AND u.scim_external_id = $2
                    "#,
                )
                .bind(org_id)
                .bind(&f.value)
...
pub async fn get_user(
    ...
    let user: UserRow = sqlx::query_as(
        ...
        FROM users u
        JOIN tenant_members om ON om.user_id = u.id
        WHERE u.id = $1 AND om.tenant_id = $2
        "#,
    )
    .bind(user_id)
    .bind(org_id)
...
pub async fn delete_user(
    ...
    sqlx::query("DELETE FROM tenant_members WHERE user_id = $1 AND tenant_id = $2")
        .bind(user_id)
        .bind(org_id)
        .execute(state.auth_service.db.pool())
        .await
...
pub async fn list_groups(
    ...
    let groups: Vec<ScimGroupRecord> = sqlx::query_as(
        r#"
        SELECT id, tenant_id as organization_id, display_name, external_id, role_id, description, created_at, updated_at
        FROM scim_groups
        WHERE tenant_id = $1
...
pub async fn create_group(
    ...
    let existing: bool = sqlx::query_scalar(
        "SELECT EXISTS(SELECT 1 FROM scim_groups WHERE tenant_id = $1 AND display_name = $2)"
    )
    .bind(org_id)
...
pub async fn patch_group(
    ...
    // Verify group exists
    let exists: bool = sqlx::query_scalar(
        "SELECT EXISTS(SELECT 1 FROM scim_groups WHERE id = $1 AND tenant_id = $2)"
    )
    .bind(group_id)
    .bind(org_id)
    .fetch_one(state.auth_service.db.pool())
...
pub async fn create_scim_token(
    ...
    let token_id: Uuid = sqlx::query_scalar(
        r#"
        INSERT INTO scim_tokens (tenant_id, name, token_hash, token_prefix, expires_at)
        VALUES ($1, $2, $3, $4, $5)
        RETURNING id
        "#,
    )
...
pub async fn revoke_scim_token(
    ...
    let result = sqlx::query(
        "UPDATE scim_tokens SET is_active = false, revoked_at = NOW() WHERE id = $1 AND tenant_id = $2"
    )
    .bind(token_id)
    .bind(org_id)
    .execute(state.auth_service.db.pool())
