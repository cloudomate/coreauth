...
impl UserRepository {
    ...
    pub async fn create(&self, new_user: &NewUser, password_hash: &str) -> Result<User> {
        ...
        let user = sqlx::query_as::<_, User>(
            r#"
            INSERT INTO users (default_tenant_id, email, password_hash, phone, metadata)
            VALUES ($1, $2, $3, $4, $5)
            RETURNING *
            "#,
        )
    ...
    pub async fn find_by_email(&self, default_tenant_id: Uuid, email: &str) -> Result<User> {
        let user = sqlx::query_as::<_, User>(
            "SELECT * FROM users WHERE default_tenant_id = $1 AND email = $2",
        )
    ...
    pub async fn find_by_phone(&self, default_tenant_id: Uuid, phone: &str) -> Result<User> {
        let user = sqlx::query_as::<_, User>(
            "SELECT * FROM users WHERE default_tenant_id = $1 AND phone = $2",
        )
        .bind(default_tenant_id)
        .bind(phone)
        .fetch_optional(&self.pool)
    ...
    pub async fn count(&self, default_tenant_id: Uuid) -> Result<i64> {
        let count: (i64,) =
            sqlx::query_as("SELECT COUNT(*) FROM users WHERE default_tenant_id = $1")
                .bind(default_tenant_id)
                .fetch_one(&self.pool)
                .await?;
