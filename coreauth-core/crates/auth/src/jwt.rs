...
pub struct Claims {
    ...
    // OAuth2 fields (present in RS256 tokens, ignored for HS256)
...
fn default_access_token_type() -> TokenType {
    TokenType::Access
}
...
pub struct JwtService {
    ...
    algorithm: Algorithm,
    /// Optional RS256 decoding key for validating OAuth2-issued tokens
    rs256_decoding_key: Option<DecodingKey>,
...
impl JwtService {
    ...
    /// Add RS256 public key for validating OAuth2-issued tokens
    pub fn with_rs256_public_key(mut self, public_key_pem: &str) -> Self {
        match DecodingKey::from_rsa_pem(public_key_pem.as_bytes()) {
            Ok(key) => { self.rs256_decoding_key = Some(key); }
            Err(e) => { tracing::warn!("Failed to load RS256 public key: {}", e); }
        }
        self
    }
    ...
    pub fn generate_access_token(
        ...
        let claims = Claims {
            ...
            iss: None, aud: None, azp: None, scope: None, org_id: None,
        };
    ...
    /// Validate and decode a token (supports both HS256 and RS256)
    pub fn validate_token(&self, token: &str) -> Result<Claims> {
        // Peek at the token header to determine the algorithm
        let header = jsonwebtoken::decode_header(token)?;

        let (validation, key) = match header.alg {
            Algorithm::RS256 => {
                if let Some(ref rs_key) = self.rs256_decoding_key {
                    let mut v = Validation::new(Algorithm::RS256);
                    // OAuth2 tokens have aud=[client_id]; skip validation here
                    v.validate_aud = false;
                    (v, rs_key)
                } else {
                    return Err(AuthError::InvalidToken("RS256 tokens not configured".into()));
                }
            }
            _ => (Validation::new(self.algorithm), &self.decoding_key),
        };
