...
impl AuthService {
    ...
    pub async fn login(&self, request: LoginRequest) -> Result<AuthResponse> {
        ...
        let user = self
            ...
            .await
            .map_err(|_| AuthError::InvalidCredentials("User not found".into()))?;
        ...
        if security.enforce_sso {
            return Err(AuthError::SsoRequired(
                "This organization requires SSO login. Password authentication is disabled.".into()
            ));
        }
        ...
        let password_hash = user
            .password_hash
            .as_ref()
            .ok_or(AuthError::InvalidCredentials("Invalid credentials".into()))?;
        ...
        if let Some(org_id) = org_id {
            ...
            if let Some((settings_json,)) = org_settings {
                ...
                if let Ok(settings) = serde_json::from_value::<ciam_models::OrganizationSettings>(settings_json) {
                    // Check SSO enforcement
                    if settings.security.enforce_sso {
                        return Err(AuthError::SsoRequired(
                            "This organization requires SSO login. Password authentication is disabled.".into()
                        ));
                    }
