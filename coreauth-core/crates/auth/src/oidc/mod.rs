...
impl OidcService {
    ...
    pub async fn get_authorization_url(
        ...
        if !provider.is_enabled {
            return Err(AuthError::InvalidCredentials("Provider is disabled".into())); // Provider is disabled
        }
    ...
    pub async fn handle_callback(
        ...
        let oauth_state: OAuthState = self
            ...
            .await
            ?
            .ok_or(AuthError::InvalidCredentials("Invalid OAuth state".into()))?;
        ...
        let email = if let Some(email) = id_token_claims.email().map(|e| e.as_str().to_string()) {
            email
        } else {
            // For Azure AD/Entra ID, email might be in preferred_username, upn, or unique_name
            // Parse the raw JWT to find alternative email claims
            let jwt_string = id_token.to_string();
            self.extract_email_from_jwt(&jwt_string)?
        };
        ...
        // Check if user is in the allowed group (if configured)
        if let Some(ref allowed_group_id) = provider.allowed_group_id {
            if !user_groups.contains(allowed_group_id) {
                tracing::warn!(
                    "User {} denied access: not a member of allowed group {}",
                    email,
                    allowed_group_id
                );
                return Err(AuthError::ExternalProviderError(
                    format!("Access denied: You are not a member of the authorized group. Contact your administrator for access.")
                ));
            }
            tracing::info!("User {} is a member of allowed group {}", email, allowed_group_id);
        }
        ...
        {
            // Ensure user is a tenant member (idempotent)
            self.ensure_tenant_member(existing_user.id, tenant_id).await?;
        ...
        // Add new user to tenant as "member" by default
        self.ensure_tenant_member(user.id, tenant_id).await?;

        // Sync groups for new user (may upgrade to admin if in admin group)
    ...
    async fn get_provider(&self, tenant_id: Uuid, provider_id: Uuid) -> Result<OidcProvider> {
        let provider = sqlx::query_as::<_, OidcProvider>(
            "SELECT * FROM oidc_providers WHERE id = $1 AND tenant_id = $2 AND is_enabled = true",
        )
        .bind(provider_id)
        .bind(tenant_id)
    ...
    pub async fn list_providers(&self, tenant_id: Uuid) -> Result<Vec<OidcProvider>> {
        let providers = sqlx::query_as::<_, OidcProvider>(
            "SELECT * FROM oidc_providers WHERE tenant_id = $1 AND is_enabled = true ORDER BY name",
        )
        .bind(tenant_id)
    ...
    pub async fn create_provider(
        ...
        allowed_group_id: Option<String>,
    ) -> Result<OidcProvider> {
        ...
        let provider = sqlx::query_as::<_, OidcProvider>(
            r#"INSERT INTO oidc_providers
               (tenant_id, name, provider_type, issuer, client_id, client_secret,
                ...
                groups_claim, group_role_mappings, allowed_group_id)
    ...
    async fn link_user_to_provider(
        ...
        sqlx::query(
            r#"INSERT INTO user_identities (user_id, provider, provider_user_id)
               VALUES ($1, $2, $3)
    ...
    async fn ensure_tenant_member(&self, user_id: Uuid, tenant_id: Uuid) -> Result<()> {
        sqlx::query(
            r#"INSERT INTO tenant_members (id, tenant_id, user_id, role, joined_at)
    ...
    fn extract_user_metadata(
        ...
        let full_name = match (&first_name, &last_name) {
            ...
            (None, None) => claims.name().and_then(|n| n.get(None)).map(|s| s.as_str().to_string()),
        };
    ...
    /// Extract email from JWT by checking alternative claims (for Azure AD/Entra ID)
    /// Azure AD may use: preferred_username, upn, unique_name instead of email
    fn extract_email_from_jwt(&self, jwt_str: &str) -> Result<String> {
        ...
        let payload_bytes = base64::engine::general_purpose::URL_SAFE_NO_PAD
            .decode(parts[1])
            .map_err(|e| AuthError::ExternalProviderError(format!("Failed to decode JWT payload: {}", e)))?;

        let payload: Value = serde_json::from_slice(&payload_bytes)
            .map_err(|e| AuthError::ExternalProviderError(format!("Failed to parse JWT payload: {}", e)))?;

        // Try alternative email claims in order of preference
        // Azure AD/Entra ID commonly uses these:
        let email_claims = ["email", "preferred_username", "upn", "unique_name"];
            ...
            if let Some(value) = payload.get(*claim) {
                if let Some(email_str) = value.as_str() {
                    ...
                    if email_str.contains('@') {
                        tracing::info!("Found email in '{}' claim: {}", claim, email_str);
        ...
        let available_claims: Vec<&str> = payload.as_object()
            .map(|obj| obj.keys().map(|k| k.as_str()).collect())
            .unwrap_or_default();
        tracing::error!(
            "Email not found in ID token. Available claims: {:?}",
            available_claims
        );

        Err(AuthError::ExternalProviderError(
            format!("Email not found in ID token. Tried claims: {:?}. Available claims: {:?}. Ensure your Azure AD app registration includes the 'email' claim or 'User.Read' scope.",
                email_claims, available_claims)
        ))
    }
