...
fn okta_template() -> OidcProviderTemplate {
    OidcProviderTemplate {
        provider_type: "okta".to_string(),
        display_name: "Okta".to_string(),
        authorization_endpoint: "https://{domain}/oauth2/default/v1/authorize".to_string(),
        token_endpoint: "https://{domain}/oauth2/default/v1/token".to_string(),
        userinfo_endpoint: Some("https://{domain}/oauth2/default/v1/userinfo".to_string()),
        jwks_uri: "https://{domain}/oauth2/default/v1/keys".to_string(),
        scopes: vec![
            "openid".to_string(),
            "profile".to_string(),
            "email".to_string(),
        ...
        groups_claim: "groups".to_string(),
        issuer_pattern: "https://{domain}/oauth2/default".to_string(),
        instructions: "Replace {domain} with your Okta domain (e.g., dev-12345.okta.com). In Okta Admin Console, create an OIDC Web Application. Set the Sign-in redirect URI to your callback URL. Under API > Authorization Servers > default, add a 'groups' claim (type: Filter, matches regex '.*') to include groups in ID tokens.".to_string(),
    }
}
...
mod tests {
    ...
    #[test]
    fn test_get_okta_template() {
        let okta = get_provider_template("okta").unwrap();
        assert_eq!(okta.provider_type, "okta");
        assert!(okta.authorization_endpoint.contains("{domain}"));
    }
