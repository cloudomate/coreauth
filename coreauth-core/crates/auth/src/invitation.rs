...
impl InvitationService {
    ...
    pub async fn create_invitation(
        ...
        let existing_user: Option<(Uuid,)> = sqlx::query_as(
            ...
            SELECT u.id
            FROM users u
            INNER JOIN tenant_members om ON u.id = om.user_id
            WHERE u.email = $1 AND om.tenant_id = $2
            "#,
        )
        .bind(email)
        .bind(tenant_id)
        ...
        variables.insert("invited_by_name".into(), inviter_name.clone());
        variables.insert("tenant_name".into(), tenant_name.clone());
        variables.insert("invitation_link".into(), invitation_link.clone());
        variables.insert("role_name".into(), role_name.clone().unwrap_or_else(|| "member".to_string()));
        ...
        let custom_template = crate::email::templates::fetch_custom_template(
            self.db.pool(),
            tenant_id,
            crate::email::templates::EmailTemplateType::UserInvitation,
        ).await;
    ...
    pub async fn accept_invitation(
        ...
        let user_id: Uuid = sqlx::query_scalar(
            r#"
            INSERT INTO users
                (default_tenant_id, email, password_hash, email_verified, is_active, metadata)
            VALUES ($1, $2, $3, $4, $5, $6)
            RETURNING id
            "#,
        )
