...
impl EmailService {
    ...
    /// Create a console-only email service that logs emails instead of sending
    pub fn console_only() -> Self {
        Self {
            provider: EmailProvider::Console {
                from_email: "noreply@ciam.dev".to_string(),
                from_name: "CIAM System".to_string(),
            },
        }
    }
    ...
    pub async fn send(&self, email: EmailMessage) -> Result<()> {
        match &self.provider {
            ...
            EmailProvider::Console { from_email, from_name } => {
                // Log the email instead of sending
                tracing::info!(
                    "📧 [CONSOLE EMAIL] From: {} <{}> To: {} Subject: {}",
                    from_name, from_email, email.to, email.subject
                );
                tracing::debug!("📧 [CONSOLE EMAIL] Body:\n{}", email.text_body);
                if let Some(html) = &email.html_body {
                    tracing::debug!("📧 [CONSOLE EMAIL] HTML Body:\n{}", html);
                }
                Ok(())
            }
        }
    ...
    /// Simple send method for quick emails
    pub async fn send_simple(&self, to: &str, subject: &str, html_body: &str) -> Result<()> {
        self.send(EmailMessage {
            to: to.to_string(),
            to_name: None,
            subject: subject.to_string(),
            text_body: html_body.to_string(), // Use HTML as text fallback
    ...
    pub async fn test_connection(&self) -> Result<()> {
        match &self.provider {
            ...
            EmailProvider::Console { .. } => {
                ...
                tracing::info!("Console email provider is ready (logs only)");
                Ok(())
            }
        }
    }
