...
export default function Connections() {
  ...
  // The backend returns default_tenant_id
  ...
  const handleSubmit = async (e) => {
    ...
    if (providerType === 'azuread' || providerType === 'azure_ad' || providerType === 'microsoft') {
      ...
      if (!formData.azure_group_id) {
        setError('Azure Group ID is required to restrict access');
        return;
      }
    ...
    const requestBody = {
      ...
      // Azure AD group restriction
      allowed_group_id: formData.azure_group_id || null,
    };
                  ...
                  <div className="flex items-center space-x-4">
                    ...
                    <div>
                      ...
                      <div className="text-xs text-slate-500 space-y-0.5 mt-1">
                        ...
                        {conn.allowed_group_id && (
                          <p>Group: {conn.allowed_group_id.substring(0, 8)}...</p>
                        )}
                        {conn.okta_domain && (
              ...
              <form onSubmit={handleSubmit} className="p-6 space-y-6">
                ...
                {(selectedTemplate.provider_type === 'azuread' || selectedTemplate.provider_type === 'azure_ad' || selectedTemplate.provider_type === 'microsoft') && (
                  <>
                    ...
                    <div>
                      <label className="block text-sm font-medium text-slate-700 mb-2">
                        Allowed Group ID <span className="text-red-500">*</span>
                      </label>
                      <input
                        type="text"
                        name="azure_group_id"
                      ...
                      />
                      <p className="text-xs text-slate-500 mt-1">
                        Only users in this Azure AD group can sign in. Found in Azure Portal → Microsoft Entra ID → Groups → Select group → Object ID
                      </p>
                    </div>
                    <div>
                      <label className="block text-sm font-medium text-slate-700 mb-2">
          ...
          <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
            <div className="bg-white rounded-lg max-w-lg w-full">
              ...
              <div className="p-6 space-y-4">
                <div className="flex items-center justify-between py-3 border-b border-slate-100">
                  ...
                  <span className={`px-3 py-1 rounded-full text-xs font-medium ${
                    ...
                    {selectedConnection.is_enabled ? 'Active' : 'Inactive'}
                  </span>
                </div>
              ...
              <div className="p-6 border-t border-slate-200 space-y-3">
                <button
                  onClick={() => handleToggleConnection(selectedConnection.id, selectedConnection.is_enabled)}
                ...
                >
                  {selectedConnection.is_enabled ? 'Disable Connection' : 'Enable Connection'}
                </button>
                <button
