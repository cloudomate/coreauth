...
impl OrganizationRepository {
    ...
    pub async fn create(&self, request: &CreateOrganization) -> Result<Organization> {
        ...
        let org = sqlx::query_as::<_, Organization>(
            r#"
            INSERT INTO organizations (slug, name, parent_organization_id, isolation_mode, custom_domain, settings)
            VALUES ($1, $2, $3, $4, $5, $6)
            RETURNING *
            "#,
        )
        .bind(&request.slug)
        .bind(&request.name)
        .bind(&request.parent_organization_id)
        .bind(&isolation_mode)
    ...
    pub async fn get_descendants(&self, org_id: Uuid) -> Result<Vec<Organization>> {
        let orgs = sqlx::query_as::<_, Organization>(
            r#"
            SELECT child.* FROM organizations parent
            JOIN organizations child ON child.hierarchy_path LIKE parent.hierarchy_path || '/%'
            WHERE parent.id = $1
    ...
    pub async fn count(&self, parent_id: Option<Uuid>) -> Result<i64> {
        let count: (i64,) = match parent_id {
            Some(parent) => {
                sqlx::query_as("SELECT COUNT(*) FROM organizations WHERE parent_organization_id = $1")
                    .bind(parent)
                    .fetch_one(&self.pool)
                    .await?
            }
            None => {
                sqlx::query_as("SELECT COUNT(*) FROM organizations WHERE parent_organization_id IS NULL")
                    .fetch_one(&self.pool)
                    .await?
            }
        };
