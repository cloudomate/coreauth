...
async fn handle_refresh_token_grant(
    ...
    let tokens = state
        .oauth2_service
        .refresh_tokens(refresh_token, application.id, application)
        .await
        .map_err(|e| {
...
async fn handle_client_credentials_grant(
    ...
    Err((
        StatusCode::BAD_REQUEST,
        Json(TokenError::unsupported_grant_type(
            "client_credentials grant not yet implemented",
        )),
    ))
}
...
pub async fn revoke(
    ...
    let _ = state
        .oauth2_service
        .revoke_token(&request.token, application.id)
        .await;
